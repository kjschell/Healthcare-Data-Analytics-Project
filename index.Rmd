...
As you can see above, my name is Kevin Schell. I am a practicing physical therapist with over fourteen years of clinical experience working with adults admitted to a major academic medical center in Boston. I am a board-certified clinical specialist in geriatric physical therapy and have provided physical therapy care to adults across all specialties including critical care environments and the hospital's emergency department. In fact, my time working in the emergency department ignited a new passion for me to take my clinical experience and (hopefully) positively impact the patient experience at a different level from one-on-one patient care. To this end, I completed my [Master of Science in Healthcare Data Analytics at the MGH Institute of Health Professions](https://www.mghihp.edu/healthcare-leadership/degrees-programs/ms-healthcare-data-analytics). I currently work as a healthcare data analyst.

To supplement my learning and growth, I have used different publicly available datasets to practice visualization techniques while looking for potential associations to further investigate with statistical analyses. Most recently, I utilized [Employee Attrition from Healthcare](https://www.kaggle.com/datasets/jpmiller/employee-attrition-for-healthcare) from Kaggle (Miller, 2023). In particular, I worked through logistic regression modeling to identify independent risk factors associated with healthcare works leaving their jobs. I completed these efforts using R (R Core Team, 2024) via R Studio for Windows version 2024.4.1.748 (Posit Team, 2024). Please see my acknowledgements and citations for all packages utilized for this analysis. 

Please feel free to peruse the rest of this site to see my efforts related to this project.
...
library(gtsummary)
setwd("C:\\Users\\kjsch\\Documents\\Schell Data Analytics Portfolio\\Spring 2024 Data Analytics Project")
...
Based on the basic descriptive statistics, the sample at large reflects a relatively young group of workers that skews older, live relatively close to home, and earn an average hourly rate well above the average for US healthcare workers ([US Bureau of Labor Statistics](https://www.bls.gov/ooh/healthcare/home.htm), 2023). Additionally, a majority of participants have worked at only one company, however, the sample is skewed greatly by those who have worked for several organizations. Interestingly, a vast majority of participants have been promoted within the last 2 years. 
...
The sample is imbalanced across the outcome of interest, Attrition. Additionally, male gender is grossly overrepresented. Nurse is the most common role identified, and a slim participants are single. More participants report lower job levels as compared to higher job levels. Interestingly, most participants rate their work and life balance positively. 
...
ggplot(data = att, aes(x = JobRole, fill = Gender)) +
  ...
  labs(
    ...
    subtitle = "Male Gender Overrepresented in Nursing and Other FGroups"
  )
```
...
```{r}
att$EnvironmentSatisfaction <- factor(att$EnvironmentSatisfaction, ordered = FALSE)
att$JobInvolvement <- factor(att$JobInvolvement, ordered = FALSE)
att$JobLevel <- factor(att$JobLevel, ordered = FALSE)
att$JobSatisfaction <- factor(att$JobSatisfaction, ordered = FALSE)
att$PerformanceRating <- factor(att$PerformanceRating, ordered = FALSE)
att$RelationshipSatisfaction <- factor(att$RelationshipSatisfaction, ordered = FALSE)
att$WorkLifeBalance <- factor(att$WorkLifeBalance, ordered =  FALSE)
att$Education <- factor(att$Education, ordered = FALSE)
att$BusinessTravel <- factor(att$BusinessTravel, ordered = FALSE)
att$Department <- factor(att$Department, ordered = FALSE)
att$EducationField <- factor(att$EducationField, ordered = FALSE)
...
No participants rated their JobPerformance below a 3. Fortunately, I did not include this variable in any of my models. 
Unfortunately, the reference Admin RoleGroup has no participants with left their job. Additionally, the Therapist RoleGroup only had 4 participants leave their job. These marked imbalances led to unrealistic exponentiated coefficients and infinite confidence intervals for the role groups. I could handle this several different ways. I can completely remove RoleGroup from future models. I could also limit the scope of the analysis to those groups with adequate numbers of observations across groups. I could also consider bootsrapping. In this case, I will plan on simply removing RoleGroup going forward. 
...
I do not see any clear evidence of these observations being abnormal or inaccurate. Given this, I would maintain these observations in my model. However, this model as a whole demonstrated correlations among continuous independent variables. Additionally, there were issues with linearity of the relationship between the continuous independent variables and the probability estimates. I also identified that RoleGroup lacked sufficient observations who left their jobs across the different levels. At least one new model will be specified to address these issues. 


## Updated Models {.tabset}

Based on the diagnostic testing of assumptions for reg1, I will remove Age, YearsAtCompany, and RoleGroup. 
...
Similar to the issue of Age in the initial model iterations, I now have evidence that TotalWorkingYears does not fit the logistic regression model well. Additionally, TotalWorkingYears demonstrated moderate correlation with YearsInCurrentRole. I will generate an additional model with this variable removed and compare its performance against reg5.
...
While reg5 appears to have a better model fit by AIC and pseudo-R^2, I will go forward with reg6 given my concerns over including TotalWorkingYears.

### Diagnostic Testing of Assumptions Reg6
...
## Interpreting Results Reg6  {.tabset}
...
With regard to commuting distances, an increase in Distance increases the odds of Attrition. More specifically, for every one mile increase in Distance, Attrition is predicted to change on average by 1.08 times. In other words, the odds of Attrition are predicted to increase by 8% on average when controlling for all other variables in the model. 

In contrast to Distance, an increase in the time an individual has spent in their current role reduces the odds of Attrition. Specifically, for every one year increase in YearsInCurrentRole, the odds of Attrition are predicted to change by 0.74 times on average when controlling for all other variables in the model. In other words, for every one year increase in YearsInCurrentRole, the odds of leaving one's role decrease by 26% on average. 

Increasing the time since a promotion increases the odds of Attrition. In this sample, for every one year increase in YearsSinceLastPromotion is predicted to change the odds of Attrition by 1.15 on average when controlling for all the other variables in the model. This means that there is a predicted 15% average increase in the odds of Attrition. 

With regard to environmental satisfaction, the odds of Attrition are predicted to change by 0.30 times on average for those who respond with level 2 as compared to those who reply with level 1 when controlling for all other variables in the model. In other words, respondents with level 2 are predicted to have odds 70% lower than those with level 1. As expected, those with even higher reported environmental satisfactor have lower odds for Attrition when controlling for all other variables.

Participating in overtime work appears to greatly impact the odds of Attrition in this sample. Those who report working overtime are predicted on average to have 25.89 times the odds of Attrition as compared to those who do not report working overtime. 

Being married or single increase the odds of Attrition when compared to those who are divorced in this sample. More specifically, those who are married have odds of Attrition 1.62 times on average as compared to those who are divorced when controlling for all other variables in the model. In contrast, those who are single are predicted to have odds 9.19 times on average those who are divorced.  

Increasing response levels on the question of JobInvolvement result in lower odds of Attrition as compared to the lowest level. For example, people reporting the highest job involvement have odds 95% lower than those with the lowest involvment in this sample.

Within this sample, higher levels of JobSatisfication resulted in lower odds of Attrition when compared to a level 1 rating. Levels 2, 3, and 4 had odds 0.54, 0.55, and 0.29 on average times the odds of level 1 when controlling for all other variables in the model.

Lastly, higher JobLevels demonstrated relatively lower odds of Attrition when compared to the lowest job level in this sample. 


### Population Level

Instead of only looking at the exponentiated point estimate for the coefficients, we will look at the 95% confidence intervals around the exponentiated coefficients in order to gain insight into the impact of the included variables on the odds of Attrition.

From the results it appears that for each one mile increase in distance from home the odds of Attrition increase from at least 1.05 to 1.11 times on average when controlling for all other variables in the model. In other words, the odds of Attrition increase on average between 5% and 11%. 

We are 95% confident that for every one year increase in the time spent in a current role, the odds of leaving a job change by 0.66 times to 0.83 times, controlling for all other independent variables in the model. In other words, the odds of Attrition decrease on average between 8% and 9%.


In contrast, for every one year increase in time since the last promotion, the odds for attrition change by 1.02 times to 1.30 times. In order words, the odds increase between 2% and 30% on average for every one year increase and when controlling for all other variables in the model. 


Working overtime strongly predicts Attrition as evidenced by an increase in odds between 14.79 to 45.30 times when controlling for all other variables.

In contrast, the odds of Attrition change by 4.55 to 18.54 times for those who report being single as compared to those who report being divorced.

Regarding job satisfaction, only the highest level of job satisfaction appears to have a clear difference when compared to the lowest level of satisfaction. On average, having the highest job satisfaction changes the odds by 0.15 to 0.56 when controlling for all other variables.

In contrast, all job levels above the lowest appear to have lower odds of Attrition. The highest job level changes the odds of Attrition by 0.01 to 0.23 times the lowest job level when controlling for all other variables. 


## References and Acknowledgements 

Achim Zeileis, Torsten Hothorn (2002).
  Diagnostic Checking in Regression
...
Fox J, Weisberg S (2019). _An R Companion to
  Applied Regression_, Third edition. Sage,
  Thousand Oaks CA.
  <https://socialsciences.mcmaster.ca/jfox/Books/Companion/>.

Iannone R, Cheng J, Schloerke B, Hughes E,
  Lauer A, Seo J (2024). _gt: Easily Create
  Presentation-Ready Display Tables_. R package
  version 0.10.1,
  <https://CRAN.R-project.org/package=gt>.

Kuhn, M. (2008). Building Predictive Models
  in R Using the caret Package. Journal of
  Statistical Software, 28(5), 1–26.
  https://doi.org/10.18637/jss.v028.i05

Long JA (2022). _jtools: Analysis and
  Presentation of Social Scientific Data_. R
  package version 2.2.0,
  <https://cran.r-project.org/package=jtools>.

Miller, J.M. (2023). Employee attrition for healthcare. Retrieved April 20, 2024 from   
...
Sjoberg DD, Whiting K, Curry M, Lavery JA,
  Larmarange J. Reproducible summary tables
  with the gtsummary package. The R Journal
  2021;13:570–80.
...
Stef van Buuren, Karin Groothuis-Oudshoorn
  ...
  Statistical Software, 45(3), 1-67. DOI
  10.18637/jss.v045.i03.

U.S. Bureau of Labor Statistics. (2024, April 17). Healthcare occupations. U.S. Bureau of Labor Statistics.
