...
```{r}
remove <- c(317, 572, 834, 973, 1084, 1221)
train.att.reg7 <- train.att %>%
  filter(EmployeeID != (remove))
dim(train.att.reg7)
reg7.no.outlier <- glm(Attrition ~ JobRole+ DistanceFromHome + YearsInCurrentRole + YearsSinceLastPromotion + EnvironmentSatisfaction + OverTime +MaritalStatus + JobInvolvement + JobSatisfaction + JobLevel, data = train.att.reg7, family = "binomial")
