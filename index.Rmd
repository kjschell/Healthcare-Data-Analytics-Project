...
library(tidyverse)
setwd("C:\\Users\\Kevin\\Documents\\Spring 2024 Data Analytics Project")
...
Out of the 34 variables included in this dataset, only JobLevel includes potentially missing values. There are 81 observations with "NA" as their value for JobLevel. It is possible that "NA" accurately reflects these observations' JobLevel. In other words, there may be 81 respondents whose roles do not include ascending levels. However, it is also possible that some respondents may have inadvertently failed to respond to this question, or there may have been a transcription error when this dataset was generated from the raw data. Without knowing how this fabricated data was "collected," it is impossible to know for certain. 

Below you will find my initial analysis of missingness using the mice package. The analysis confirmed that only JobLevel had missing values. Summaries of sdatasets with all observations and one with observations removed for missing JobLevel are included below as well. Had the differences been more egregious, I would have considered imputation of JobLevel. However, given the lack of clarity regarding the meaning of NA combined with only subtle differences and relatively small percentage of missing values, I decided I would remove the observations with missing values and use this dataset if JobLevel appeared relevant to include when specifying models. Visual analysis of Attrition by JobLevel demonstrates that a higher percentage of respondents with lower job level leave their role. Given this, I decided to go forward with the dataset with observations removed for missingness. 
...
## Descriptive Statistics, Tables, and Visualizations {.tabset}
...
ggplot(data = att, aes(Age)) +
  ...
  geom_vline(aes(xintercept = mean(Age)), color = "black", linewidth = 1.25) +
  theme_test()+
  labs(
...
ggplot(data = att, aes(x = "", y=Age)) +
  geom_boxplot() +
  geom_jitter(shape = 21, alpha = 0.25, color = "grey3") +
  theme_test() +
  theme(
  ...
  labs(
    title = "Age of Respondents",
    subtitle = "Older Skew Less Obvious",
    x = "All Respondents",
    y = "Age in Years"
  )
...
ggplot(data = att, aes(x = JobRole, fill = JobRole)) +
  geom_bar(color = "dimgrey") +
  theme_test() +
  scale_fill_manual(values = c("dodgerblue4", "hotpink4", "lightyellow2", "seagreen4")) +
  labs(
...
ggplot(data = att, aes(Age, fill = JobRole)) +
  geom_histogram(binwidth = 2, color = "dimgrey") +
...
ggplot(data = att, aes(x = JobRole, y = Age, fill = JobRole)) +
  geom_boxplot(color = "grey6") +
  theme_test() +
  labs(
...
ggplot(data = att, aes(x = JobRole, y = DistanceFromHome, fill = JobRole)) +
  geom_boxplot() +
  theme_test() +
...
ggplot(data = att, aes(x = JobRole, y = HourlyRate, fill = JobRole)) +
  geom_boxplot() +
  theme_test() +
    scale_y_continuous(breaks = round(seq(min(att$HourlyRate), max(att$HourlyRate), by = 5),1)) +
  scale_fill_manual(values = c("dodgerblue4", "hotpink4", "lightyellow2", "seagreen4")) +
  labs(
...
ggplot(data = att, aes(x = JobRole, y = PercentSalaryHike, fill = JobRole)) +
  geom_boxplot() +
  theme_test() +
    scale_y_continuous(breaks = round(seq(min(att$PercentSalaryHike), max(att$PercentSalaryHike), by = 1),1)) +
  scale_fill_manual(values = c("dodgerblue4", "hotpink4", "lightyellow2", "seagreen4")) +
  labs(
...
ggplot(data = att, aes(x = TotalWorkingYears)) +
  ...
  geom_vline(aes(xintercept = mean(TotalWorkingYears)), color = "black", linewidth = 1.25) +
  theme_test() +
...
ggplot(data = att, aes(x = JobRole, y = TotalWorkingYears, fill = JobRole)) +
  geom_boxplot() +
  theme_test() +
...
ggplot(data = att, aes(x = JobRole, y = YearsAtCompany, fill = JobRole)) +
  geom_boxplot() +
  theme_test() +
...
ggplot(data = att, aes (x = YearsInCurrentRole)) +
  ...
  geom_vline(xintercept = mean(att$YearsInCurrentRole, color = "black", linewidth = 1.25 )) +
...
ggplot(data = att, aes(x = JobRole, y = YearsInCurrentRole, fill = JobRole)) +
  geom_boxplot() +
  theme_test() +
...
ggplot(data = att, aes (x = YearsSinceLastPromotion)) +
  ...
  geom_vline(xintercept = mean(att$YearsSinceLastPromotion, color = "black", linewidth = 1.25 )) +
...
ggplot(data = att, aes(x = JobRole, y = YearsSinceLastPromotion, fill = JobRole)) +
  geom_boxplot() +
  theme_test() +
...
ggplot(data = att, aes(x = Attrition, fill = Attrition)) +
  ...
  scale_fill_manual(values = c("red4", "green4"))
  labs(
    title = "Attrition Separated by Gender",
    subtitle = "Slightly Greater Percentage of Female Gender Group Left Job"
  )
...
```{r}
ggplot(data = att, aes(x = Attrition, y = Age, fill = Attrition)) + 
  geom_boxplot() +
  theme_test() +
  scale_fill_manual(values = c("red4", "green4")) +
  labs(
...
```{r}
ggplot(data = att, aes(x = Attrition, y = DistanceFromHome, fill = Attrition)) +
...
```{r}
ggplot(data = att, aes(x = Attrition, y = HourlyRate, fill = Attrition)) +
  geom_boxplot() +
  theme_test() +
  scale_fill_manual(values = c("red4", "green4")) +
  labs(
...
```{r}
ggplot(data = att, aes(x = Attrition, y = PercentSalaryHike, fill = Attrition)) +
  geom_boxplot(color = "dimgrey") +
  theme_test() +
  scale_fill_manual(values = c("red4", "green4")) +
  labs(
...
```{r}
ggplot(data = att, aes(x = Attrition, y = TotalWorkingYears, fill = Attrition)) +
  geom_boxplot(color = "dimgrey") +
  theme_test() +
  scale_fill_manual(values = c("red4", "green4")) +
  labs(
...
#### Years at Current Company 
```{r}
ggplot(data = att, aes(x = Attrition, y = YearsAtCompany, fill = Attrition)) +
  geom_boxplot(color = "dimgrey") +
  theme_test() +
  scale_fill_manual(values = c("red4", "green4")) +
  labs(
...
```{r}
ggplot(data = att, aes(x = Attrition, y = YearsInCurrentRole, fill = Attrition)) +
  geom_boxplot(color = "dimgrey") +
  theme_test()+
  scale_fill_manual(values = c("red4", "green4")) +
  labs(
...
#### Years Since Last Promotion
```{r}
ggplot(data = att, aes(x = Attrition, y = YearsSinceLastPromotion, fill = Attrition)) +
  geom_boxplot(color = "dimgrey") +
  theme_test() +
  scale_fill_manual(values = c("red4", "green4")) +
  labs(
...
### Reg1 Performance on Test Data

```{r}
if (!require(caret)) install.packages("caret")
library(caret)
test.probabilities <- predict(reg1, test.att, type = "response")
...
Based on the above plots, there are little to no relationships between the residuals and the continuous independent variables in reg1. The p-values for the hypothesis tests related to this question are above 0.5. For now, I will not reject my null hypothesis that the continuous independent variables fit the logisti regression model well. 

### Linearity of Relationshipe Between Continuous Ind Var and Natural Logs of Odds
...
When considering the Variance Inflation Factor, YearsAtCompany is relatively large. I think it may be most reasonable to generate a new model by retaining all of the indepnedent variables save for YearsAtCompany. 
...
Observations 328, 349, 964, 973, 1012, and 1221 demonstrated significant influence. When I re-specify the model without the YearsAtCompany variable, I will run it both with and without these observations. 

## Updated Dataset and Model
```{r}
remove <- c(328, 349, 964, 973, 1012, 1221)
train.att.No.Outlier<- train.att %>%
  filter(!EmployeeID %in% remove)
...
linearity.check.new <-train.att.No.Outlier %>%
